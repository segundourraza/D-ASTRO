...
density_analysis = simInputs.density_analysis;
...
if density_analysis
    ...
    density_label = density_label{simInputs.densityMode};
    ndensity = 1;
...
switch simInputs.AerocaptureCorridor
    ...
    case 2  % AEROCAPTURE CORRIDOR SERIAL IMPLEMENTATION
        for ibc = 1:nBC % Looping BC range
            ...
            tempTraj = zeros([simInputs.nPoints, 3*2*10]);
            ...
            for density_mode = 1:ndensity
                results = zeros([9,2]);
                drivers = zeros([7,2]);
                fprintf("\tDensity scenario " + density_mode + ": " + density_label{density_mode} + ".")
